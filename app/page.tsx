...
export default function Home({canWrite=false,onLock=()=>{}}:{canWrite?:boolean;onLock?:()=>void}){
 ...
 const syncTimer=useRef<ReturnType<typeof setTimeout>|null>(null),visitorId=useRef('');
 ...
 useEffect(()=>{alive.current=true;let cancelled=false;void (async()=>{
  ...
  try{
   ...
   current.current=display;setEntries(display);setEditId(display[0]?.id||'');
 ...
 const [query,setQuery]=useState(''),[selected,setSelected]=useState('all'),[font,setFont]=useState(19);
 ...
 const visible=entries.filter(e=>(selected==='all'||e.id===selected)&&(!query||`${e.body}${e.date}`.includes(query)));
 return <main className={`cottage season-${season} time-${time}`}>
  ...
  <aside className="sidebar">
   <button className="brand" onClick={()=>{setSelected('all');setQuery('');}}><span>⌂</span>日記小屋</button><p className="subtitle">把日子寫給你看</p>
   ...
   <div className="calendar" aria-label={`${year}年${monthNumber}月日曆`}>{'日一二三四五六'.split('').map(d=><span className="weekday" key={d}>{d}</span>)}{Array.from({length:firstWeekday},(_,i)=><span key={`blank${i}`}/>)}{Array.from({length:daysInMonth},(_,i)=>{const entry=entries.find(e=>e.iso===`${month}-${String(i+1).padStart(2,'0')}`);return <button key={i} disabled={!entry} className={`${entry?'has-entry':''} ${entry?.id===(mode==='read'?selected:editId)?'chosen':''}`} onClick={()=>{if(entry){setSelected(entry.id);setEditId(entry.id);}}} aria-label={`${monthNumber}月${i+1}日${entry?'，有日記':''}`}>{i+1}</button>;})}</div>
   <nav aria-label="日記日期"><button className={selected==='all'?'active':''} onClick={()=>{setSelected('all');setMode('read');}}>☷ <span>所有日記</span><small>{entries.length}</small></button>{entries.filter(e=>e.iso.startsWith(month)).map(e=><button key={e.id} className={(mode==='write'?editId:selected)===e.id?'active':''} onClick={()=>{setSelected(e.id);setEditId(e.id);}}>♧ <span>{e.date}</span><small>{data.likes.includes(e.id)?'♥':''}</small></button>)}</nav>
  ...
  <section className="journal" aria-label="日記內容" style={{'--reading-size':`${font}px`} as React.CSSProperties}>
   ...
   </article>)}{!visible.length&&<div className="empty"><h2>還沒找到這段回憶</h2><p>換個關鍵字試試吧。</p><button onClick={()=>{setQuery('');setSelected('all');}}>回到所有日記</button></div>}</div>}
