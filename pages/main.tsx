...
function App() {
  const [session, setAccess] = useState<{role: 'reader' | 'owner'; expiresAt: number} | null>(null);
  const [role, setRole] = useState<'reader' | 'owner'>('reader');
  const [password, setPassword] = useState('');
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState('');
  function lock() { clearSession(); setAccess(null); setPassword(''); }
  useEffect(() => {
    const expired = () => { lock(); setError('小屋已上鎖，請重新輸入密碼。未完成的草稿仍在這個分頁的記憶體中。'); };
    window.addEventListener('cottage-expired', expired);
    const timer = session ? setTimeout(expired, Math.max(0, session.expiresAt - Date.now())) : undefined;
  ...
  return <main className="password-screen"><form className="password-card" onSubmit={async e => {
    ...
    try {
      ...
      setSession(result.token); setPassword(''); setAccess(result);
    ...
    <small>日記會在密碼驗證成功後才載入。<br />重新整理或關閉分頁後，需要再次輸入密碼。</small>
  </form></main>;
}
